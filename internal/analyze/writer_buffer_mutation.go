...
func isWriterMethod(function *ssa.Function) bool {
	...
	if function.Name() != "Write" || signature.Recv() == nil ||
		signature.Params().Len() != 1 || signature.Results().Len() != 2 ||
		signature.Variadic() || len(function.Params) < 2 {
		return false
	}
...
func modifiesWriterBuffer(instruction ssa.Instruction, buffer ssa.Value) bool {
	...
	case *ssa.Call:
		...
		return ok && builtin.Name() == "append" && len(instruction.Common().Args) != 0 &&
			instruction.Common().Args[0] == buffer
	default:
		return false
	}
}
