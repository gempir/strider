...
func (l *layout) markAnyDelimited(node cst.Node, opens []int, closes []bool) {
	tokens := cst.NodeTokens(node)
	if len(tokens) < 2 {
		return
	}
...
func (l *layout) markDelimited(node cst.Node, openKind, closeKind token.Token, opens []int, closes []bool) {
	open, close := -1, -1
...
func (l *layout) markTokens(node cst.Node, wanted token.Token, target []bool) {
	for _, child := range cst.Children(node) {
		current, ok := child.(cst.Token)
		if !ok {
			continue
...
func (l *layout) markToken(current cst.Token, target []bool) {
	if index, ok := l.tokenIndex(current); ok {
		target[index] = true
	}
}

func (l *layout) markFirstToken(node cst.Node, target []bool) {
	if node == nil {
		return
	}
	tokens := cst.NodeTokens(node)
...
func (l *layout) shouldBreak(open, close, column, width int, comments []cst.Comment, source []byte, preserveEmptyLines bool) bool {
	...
	for index := open + 1; index <= close; index++ {
		...
		if l.softOpen[index] == 0 && needsSpace(previous, current.Ch(), l.spacedOps[index] || l.spaceBefore[index]) {
			length++
		}
