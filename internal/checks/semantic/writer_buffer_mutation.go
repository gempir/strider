		...
		for _, block := range function.Blocks {
			for _, instruction := range block.Instrs {
				if modifiesWriterBuffer(instruction, buffer) {
					pass.Report(positionNode{position: instruction.Pos()}, "io.Writer.Write must not modify the provided buffer, even temporarily")
				}
			}
		}
	}
...
func isWriterMethod(function *ssa.Function) bool {
	...
	if function.Name() != "Write" || signature.Recv() == nil || signature.Params().Len() != 1 || signature.Results().Len() != 2 || signature.Variadic() || len(function.Params) < 2 {
		return false
	}
	slice, ok := signature.Params().At(0).Type().Underlying().(*types.Slice)
	if !ok || !types.Identical(slice.Elem(), types.Typ[types.Byte]) || !types.Identical(signature.Results().At(0).Type(), types.Typ[types.Int]) {
		return false
	}
