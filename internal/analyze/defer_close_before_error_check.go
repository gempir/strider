		...
		ast.Inspect(
			...
			func(node ast.Node) bool {
				...
				for index := 0; index + 1 < len(block.List); index++ {
					...
					if receiver == nil || pass.TypesInfo.ObjectOf(receiver) != pass.TypesInfo.ObjectOf(
						resource,
					) {
						continue
					}
					pass.Report(
						deferred,
						fmt.Sprintf(
							"check the error from %s before deferring %s.Close",
							renderAnalysisExpression(pass, call.Fun),
							resource.Name,
						),
					)
				}
