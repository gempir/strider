...
func cstZeroIntegerLiteralDivision(binary *cst.BinaryExpression) bool {
	...
	left, leftOK := cstUnparen(binary.LHS).(*cst.BasicLit)
	right, rightOK := cstUnparen(binary.RHS).(*cst.BasicLit)
...
func cstUnparen(node cst.Node) cst.Node {
	for {
...
func cstBooleanLiteral(node cst.Node) bool {
	spelling := cst.Spelling(cstUnparen(node))
	return spelling == "true" || spelling == "false"
}

func (a *cstAnalyzer) checkUnaryExpression(expression *cst.UnaryExpr) {
