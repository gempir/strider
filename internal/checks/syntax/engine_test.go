...
`),
	...
	if len(byCode) < 12 {
		t.Fatalf("fixture exercised only %d CST checks: %v", len(byCode), sortedFindingCodes(byCode))
	}
	for code, want := range byCode {
		...
		t.Run(
			...
			func(t *testing.T) {
				...
				gotFindings := analyzeCSTFindings("bad_file.go", tree, checks)
				...
				if !reflect.DeepEqual(got, want) {
					t.Fatalf("single-check findings differ\ngot:  %v\nwant: %v", got, want)
				}
			},
		)
	}
...
func TestAnalyzeCSTReceiverChecksAreIndependent(t *testing.T) {
	tree, err := cst.Parse(
		"receiver.go",
	...
	for _, code := range []string{
		...
		t.Run(
			...
			func(t *testing.T) {
				...
				findings := analyzeCSTFindings("receiver.go", tree, checks)
				if len(findings) == 0 {
...
func analyzeCSTFindings(filename string, tree *cst.Tree, checks []Check) []Finding {
	result := []Finding{}
	AnalyzeCST(CSTInput{
		Filename: filename,
		Tree:     tree,
...
func findingKey(finding Finding) string {
	...
	if !finding.HasRange {
		start, end = cst.Range(finding.Node)
	}
