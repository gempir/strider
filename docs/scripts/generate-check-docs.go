...
const (
	...
	scaffoldGeneratedMarker = "<!-- Code generated by scripts/checkscaffold; edit metadata in the check descriptor. -->"
...
func synchronizeCheckPages(docsDirectory string) *checks.Registry {
	...
	for _, rule := range registry.Checks() {
		...
		if strings.Contains(updated, scaffoldGeneratedMarker) || rule.Engine() == checks.EngineSemantic && strings.Contains(updated, generatedMarker) {
			updated = render(
				pageData{
					code:        meta.Code,
					summary:     meta.Summary,
		...
		updated = withSeverityMetadata(updated, meta.DefaultSeverity)
