...
func validatePrintfUses(
	...
	for _, use := range uses {
		customFormatter := use.value >= 0 && use.value < len(arguments) &&
			hasFormatMethod(pass.TypesInfo.TypeOf(arguments[use.value]))
...
func printfVerbAcceptsRecursive(
	...
	if (verb == 's' || verb == 'q' || verb == 'x' || verb == 'X') &&
		(hasStringMethod(valueType) || implementsError(valueType)) {
		return true
	}
	...
	case *types.Slice:
		if verb == 'p' || ((verb == 's' || verb == 'q' || verb == 'x' || verb == 'X') &&
			isByteType(aggregate.Elem())) {
			return true
		}
	...
	case *types.Map:
		...
		return printfVerbAcceptsRecursive(verb, aggregate.Key(), false, seen) &&
			printfVerbAcceptsRecursive(verb, aggregate.Elem(), false, seen)
	case *types.Struct:
	...
	case 'x', 'X':
		return printfStringType(valueType) ||
			printfBasicInfo(valueType, types.IsInteger|types.IsFloat|types.IsComplex) ||
...
func printfStringType(valueType types.Type) bool {
	...
	case *types.Slice:
		return printfBasicInfo(underlying.Elem(), types.IsInteger) &&
			underlying.Elem().Underlying().(*types.Basic).Kind() == types.Byte
	case *types.Array:
		return printfBasicInfo(underlying.Elem(), types.IsInteger) &&
			underlying.Elem().Underlying().(*types.Basic).Kind() == types.Byte
	default:
...
func hasStringMethod(valueType types.Type) bool {
	...
	signature, _ := function.Type().(*types.Signature)
	return signature != nil && signature.Params().Len() == 0 && signature.Results().Len() == 1 &&
		printfBasicInfo(signature.Results().At(0).Type(), types.IsString)
}
