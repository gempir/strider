		...
		ast.Inspect(
			...
			func(node ast.Node) bool {
				...
				if fixed {
					pass.Report(
						binary,
						fmt.Sprintf("comparison is always %t for type %s", truth, pass.TypesInfo.TypeOf(value)),
					)
				}
				return true
			},
...
func comparisonFixedByBounds(
	operator token.Token,
	bound, minimum, maximum constant.Value,
) (bool, bool) {
