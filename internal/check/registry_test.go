...
func TestUnifiedRegistrySupportsScopedMinimumSeverities(t *testing.T) {
	registry, err := NewRegistry(
		RegistryOptions{
			Only: []string{"format", "no-init", "suspicious-sleep"},
			Settings: map[string]config.RuleConfig{
				"format": {Severity: "warning"},
				"no-init": {Severity: "warning"},
				"suspicious-sleep": {Severity: "warning"},
			},
			MinimumSeverity: diagnostic.SeverityNote,
			LintMinimumSeverity: diagnostic.SeverityError,
			AnalyzeMinimumSeverity: diagnostic.SeverityWarning,
		},
	)
	if err != nil {
		t.Fatal(err)
	...
	rules := registry.Rules()
	if got := len(rules); got != 2 || rules[0].Meta().Code != "format" || rules[1].Meta().Code != "suspicious-sleep" {
		t.Fatalf("scoped checks = %#v, want format and suspicious-sleep", rules)
	}
}
...
func TestUnifiedRegistryRejectsInvalidMinimumSeverity(t *testing.T) {
	...
	_, err = NewRegistry(
		RegistryOptions{
			Only: []string{"format"},
			Settings: map[string]config.RuleConfig{"no-init": {Severity: "fatal"}},
		},
	)
