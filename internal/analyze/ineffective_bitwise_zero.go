...
func (ineffectiveBitwiseZeroRule) Run(pass *Pass) {
	...
	for _, file := range pass.Files {
		ast.Inspect(file, func(node ast.Node) bool {
			...
			if iotaName != "" {
				message += fmt.Sprintf(
					"; %s is zero because it is declared with iota—did you mean 1 << iota?",
					iotaName,
				)
			}
...
func bitwiseZeroOperand(
	pass *Pass,
	expression ast.Expr,
	iotaConstants map[*types.Const]bool,
) (bool, string) {
	...
	if !ok || object.Pkg() != pass.Types || !iotaConstants[object] ||
		constant.Sign(object.Val()) != 0 {
		return false, ""
	}
