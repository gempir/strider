...
func Run(shared *workspace.Workspace, registry *Registry, options RunOptions) (Result, error) {
	...
	if err := appendAnalysis(&result, shared, registry, options, analyze.Run); err != nil {
		return Result{}, err
	}
...
type analysisRunner func([]string, *analyze.Registry) ([]diagnostic.Diagnostic, error)
...
func appendAnalysis(
	...
	if registry.analyze == nil || len(registry.analyze.Rules()) == 0 {
		return nil
	}
	packageDiagnostics, err := run(shared.Inputs(), registry.analyze)
	if err != nil {
		return err
	}
...
func runConcreteFile(
	...
	filename := file.Path()
	defer file.Release()
	lintApplies := registry.lint != nil && registry.lint.Applies(filename)
	...
	if lintApplies {
		result.diagnostics = append(
			result.diagnostics,
			lint.AnalyzeTree(filename, tree, registry.lint)...,
		)
	}
	return result
}
