...
func encodeBufferDescriptor(call ssa.CallInstruction) (encodeBufferCall, bool) {
	if isStaticFunction(call, "encoding/ascii85", "Encode") || isStaticFunction(
		call,
		"encoding/hex",
		"Encode",
	) {
...
func encodeBuffersOverlap(destination, source ssa.Value) bool {
	...
	if !destinationOK || !sourceOK || flattenSSAValue(destinationSlice.X) != flattenSSAValue(
		sourceSlice.X,
	) {
		return false
	}
...
func sameSliceBound(left, right ssa.Value) bool {
	...
	return leftOK && rightOK && leftConstant.Value != nil && rightConstant.Value != nil && constant.Compare(
		leftConstant.Value,
		token.EQL,
		rightConstant.Value,
	)
