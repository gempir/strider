...
func (partiallyTypedConstantGroupCheck) Run(pass *Pass) {
	for _, file := range pass.Files {
		for _, declaration := range file.Decls {
			...
			if !ok || first.Type == nil || len(first.Names) != 1 || len(first.Values) != 1 || !constantLiteral(first.Values[0]) {
				continue
			}
			...
			pass.Report(first, "only the first constant with an explicit value has the declared type; repeat the type on the remaining constants")
		}
	}
}
...
func partiallyTypedLiteralGroup(pass *Pass, specs []ast.Spec, firstType types.Type) bool {
	for _, raw := range specs {
		...
		if !ok || specification.Type != nil || len(specification.Names) != 1 || len(specification.Values) != 1 || !constantLiteral(specification.Values[0]) {
			return false
		}
