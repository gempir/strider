...
func TestPlanComposesGranularEditsBeforeFormatting(t *testing.T) {
	...
	diagnostics := []diagnostic.Diagnostic{
		...
		automaticDiagnostic(filename, "double-negation", diagnostic.Safe, []diagnostic.TextEdit{{Start: start, End: start + 1}, {Start: start + 1, End: start + 2}}),
	}

	result, err := Plan(snapshot, diagnostics, map[string]formatter.Result{filename: candidate}, Options{Mode: SafeOnly})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlanSafeOnlyAndIncludeUnsafe(t *testing.T) {
	...
	diagnostics := []diagnostic.Diagnostic{
		automaticDiagnostic(filename, "safe", diagnostic.Safe, []diagnostic.TextEdit{{Start: safeStart, End: safeStart + len("safeName"), NewText: "fixedSafe"}}),
		automaticDiagnostic(filename, "unsafe", diagnostic.Unsafe, []diagnostic.TextEdit{{Start: unsafeStart, End: unsafeStart + len("unsafeName"), NewText: "fixedUnsafe"}}),
	}

	safe, err := Plan(snapshot, diagnostics, nil, Options{Mode: SafeOnly})
	if err != nil {
		t.Fatal(err)
	}
	...
	all, err := Plan(snapshot, diagnostics, nil, Options{Mode: IncludeUnsafe})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlanSkipsEveryNonIdenticalOverlappingFix(t *testing.T) {
	...
	start := bytes.Index(before, []byte("value"))
	diagnostics := []diagnostic.Diagnostic{
		automaticDiagnostic(filename, "first", diagnostic.Unsafe, []diagnostic.TextEdit{{Start: start, End: start + 3, NewText: "one"}}),
		automaticDiagnostic(filename, "second", diagnostic.Unsafe, []diagnostic.TextEdit{{Start: start + 2, End: start + 5, NewText: "two"}}),
	}

	result, err := Plan(snapshot, diagnostics, nil, Options{Mode: IncludeUnsafe})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlanCoalescesOnlyIdenticalAtomicFixes(t *testing.T) {
	filename, before, snapshot := fixFixture(t, "package sample\n\nvar value = 1\n")
	start := bytes.Index(before, []byte("value"))
	edits := []diagnostic.TextEdit{{Start: start, End: start + len("value"), NewText: "other"}}
	diagnostics := []diagnostic.Diagnostic{
...
func TestPlanKeepsPartiallySharedFixesAtomic(t *testing.T) {
	...
	shared := diagnostic.TextEdit{Start: start + 1, End: start + 2, OldText: "b"}
	diagnostics := []diagnostic.Diagnostic{
		automaticDiagnostic(filename, "first", diagnostic.Unsafe, []diagnostic.TextEdit{{Start: start, End: start + 1, OldText: "a"}, shared}),
		automaticDiagnostic(filename, "second", diagnostic.Unsafe, []diagnostic.TextEdit{shared, {Start: start + 2, End: start + 3, OldText: "c"}}),
	}
...
func TestPlanUsesByteOffsetsWithUTF8AndCRLF(t *testing.T) {
	filename, before, snapshot := fixFixture(t, "package sample\r\n\r\nvar label = \"λ\"\r\nvar value = 1\r\n")
	start := bytes.Index(before, []byte("value"))
	item := automaticDiagnostic(filename, "rename", diagnostic.Unsafe, []diagnostic.TextEdit{{Start: start, End: start + len("value"), NewText: "count"}})

	result, err := Plan(snapshot, []diagnostic.Diagnostic{item}, nil, Options{Mode: IncludeUnsafe})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlanRejectsInvalidRangesAndAutomaticAlternatives(t *testing.T) {
	filename, before, snapshot := fixFixture(t, "package sample\n")
	invalid := automaticDiagnostic(filename, "invalid", diagnostic.Unsafe, []diagnostic.TextEdit{{Start: 0, End: len(before) + 1}})
	if _, err := Plan(snapshot, []diagnostic.Diagnostic{invalid}, nil, Options{Mode: IncludeUnsafe}); err == nil || !strings.Contains(err.Error(), "invalid edit range") {
		t.Fatalf("invalid range error = %v", err)
	}

	alternatives := invalid
	alternatives.Fixes = append(alternatives.Fixes, alternatives.Fixes[0])
	if _, err := Plan(snapshot, []diagnostic.Diagnostic{alternatives}, nil, Options{Mode: IncludeUnsafe}); err == nil || !strings.Contains(
		err.Error(),
		"more than one automatic fix",
	) {
		t.Fatalf("automatic alternatives error = %v", err)
	}

	staleEdit := automaticDiagnostic(filename, "stale-edit", diagnostic.Unsafe, []diagnostic.TextEdit{{Start: 0, End: len("package"), OldText: "missing"}})
	if _, err := Plan(snapshot, []diagnostic.Diagnostic{staleEdit}, nil, Options{Mode: IncludeUnsafe}); err == nil || !strings.Contains(
		err.Error(),
		"did not contain expected source",
	) {
...
func TestPlanRejectsSafeFixThatDoesNotTypeCheck(t *testing.T) {
	...
	item := automaticDiagnostic(filename, "break-types", diagnostic.Safe, []diagnostic.TextEdit{{Start: start, End: start + 1, NewText: "missing"}})
	if _, err := Plan(snapshot, []diagnostic.Diagnostic{item}, nil, Options{Mode: SafeOnly}); err == nil || !strings.Contains(err.Error(), "undefined: missing") {
...
func TestApplyRejectsStaleSource(t *testing.T) {
	filename, before, snapshot := fixFixture(t, "package sample\n\nvar value = 1\n")
	start := bytes.Index(before, []byte("value"))
	item := automaticDiagnostic(filename, "rename", diagnostic.Unsafe, []diagnostic.TextEdit{{Start: start, End: start + len("value"), NewText: "other"}})
	result, err := Plan(snapshot, []diagnostic.Diagnostic{item}, nil, Options{Mode: IncludeUnsafe})
	if err != nil {
		t.Fatal(err)
	}
...
func TestApplyGuardsUnchangedAnalyzedFiles(t *testing.T) {
	...
	shared, err := workspace.Open([]string{directory}, workspace.Options{})
	if err != nil {
		t.Fatal(err)
	}
	...
	item := automaticDiagnostic(first, "rename", diagnostic.Unsafe, []diagnostic.TextEdit{{Start: start, End: start + len("firstValue"), NewText: "otherValue"}})
	result, err := Plan(snapshot, []diagnostic.Diagnostic{item}, nil, Options{Mode: IncludeUnsafe})
	if err != nil {
		t.Fatal(err)
	}
...
func TestApplyRejectsRetargetedSourceSymlink(t *testing.T) {
	...
	shared, err := workspace.Open([]string{link}, workspace.Options{})
	if err != nil {
		t.Fatal(err)
	}
	snapshot, err := Capture(shared)
	...
	start := bytes.Index(before, []byte("value"))
	item := automaticDiagnostic(link, "rename", diagnostic.Unsafe, []diagnostic.TextEdit{{Start: start, End: start + len("value"), NewText: "other"}})
	result, err := Plan(snapshot, []diagnostic.Diagnostic{item}, nil, Options{Mode: IncludeUnsafe})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlanMatchesFormatterExclusionsAgainstSourceSymlinkPath(t *testing.T) {
	...
	result, err := Plan(snapshot, []diagnostic.Diagnostic{item}, nil, Options{Mode: IncludeUnsafe, Root: resolvedRoot, FormatExcludes: []string{"linked.go"}})
	if err != nil {
		t.Fatal(err)
	}
...
func automaticDiagnostic(filename, code string, safety diagnostic.Safety, edits []diagnostic.TextEdit) diagnostic.Diagnostic {
	return diagnostic.Diagnostic{File: filename, Code: code, Fixes: []diagnostic.Fix{{Message: "apply test fix", Safety: safety, Automatic: true, Edits: edits}}}
}
